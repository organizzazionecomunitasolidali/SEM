      ...
      for (const processLazy of processArray) {
        ...
        if(process.status & PROCESS_STATUS_RUNNING){
          ...
          /* We have this issue https://github.com/organizzazionecomunitasolidali/SEM/issues/5
            ...
            time span limit , say 1 hour ago , assuming the previous execution crashed.
          */
          if(process.last_start == 0 || process.last_start < timestampMs - 1 * 3600 * 1000){
  ...
  async crawl(websiteLazy: SemWebsite) {
    ...
    try {
      while (pageUrl) {
        ...
        if(!paginationHtmlElementStructure.hasOwnProperty("json") || !paginationHtmlElementStructure.json || !JSON.parse(paginationHtmlElementStructure.json).is_infinite_scrolling){
